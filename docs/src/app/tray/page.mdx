...
1. The window declares `close_policy = "hide"` in app.zon, so the red close button hides it instead of quitting — the app keeps running behind its status item, and on macOS clicking the Dock icon re-shows the hidden window on its own. The default (`"quit"`) keeps the classic behavior; see [Windows](/windows#close-policy).
