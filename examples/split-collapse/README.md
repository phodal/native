...
Two driving modes, selected by environment variable:
...
Every tween step logs its arrival cadence on stderr (`tween-frame dt_ms=...` in manual mode, `tween-echo fraction=...` in runtime mode), so a driver can count the visible steps of the 180 ms collapse and read the real deltas between frames.
