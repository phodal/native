...
async function pageContent(href: string, fallback: string): Promise<string> {
  ...
  if (href === "/") {
    return "Build beautiful native apps. Cross-platform native UI: declarative Native markup views (.native), design tokens, Zig logic, a predictable message-based state loop, hot reload, typed effects, AI agent skills and evals, automation, WebView coexistence on macOS, Linux, Windows, iOS, and Android.";
  }
