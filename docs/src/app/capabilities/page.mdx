...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>macOS system-engine host (one player: AVAudioPlayer for local files, AVPlayer for streamed URLs), Windows system WebView (one Media Foundation media session for local files and streamed URLs), and Linux system WebView (one GStreamer playbin for local files and streamed URLs, loaded at runtime — a host without GStreamer reports unsupported); macOS Chromium and GStreamer-less Linux hosts report unsupported and playback degrades to one explicit failed Msg</td>
    </tr>
    <tr>
      <td>File drops</td>
