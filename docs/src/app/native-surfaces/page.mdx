...
Native surfaces are the app-owned regions that the Native SDK composes inside a platform window. A surface can be a WebView, native chrome, a native utility panel, or a specialized drawing region. The key model is that WebView content is one surface in the window, not the whole app.
