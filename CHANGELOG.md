...
- **Canvas + WebView in one window (`UiApp` webview panes)**: the "native, web, or both per window" claim is now exercised live. A scene can declare a child webview next to a `gpu_surface` canvas without growing an implicit `main` webview (`sceneNeedsMainWebView` now only triggers on a `main`-labeled webview; canvas-first windows stay webview-free behind the canvas), and `UiApp.Options.web_panes` maps model state onto those webviews: each pane snaps its webview to a canvas widget's layout frame (an empty panel carrying the pane's `anchor` semantics label — the region is laid out by the widget engine like everything else), navigates when the model's URL changes, and reloads when its `reload_token` bumps (the CenterPane/Preview-tab consumer shape). Frames reconcile against the runtime's *actual* webview state on every rebuild and presented frame — a shell relayout that stomps a pane frame also invalidates the canvas, so the next frame re-snaps it; nothing trusts a cache. Pane URLs pass `security.navigation.allowed_origins`; failures degrade to a taught warning, never a dead render loop. The seam is engine-agnostic by construction: it rides the same `PlatformServices` child-webview surface (`createView`/`setWebViewFrame`/`navigateWebView`) that both macOS hosts implement — WKWebView with `-Dweb-engine=system`, CEF with `-Dweb-engine=cef`/`chromium` (`cef_host.mm` implements the identical symbol surface and was kept in sync + clang-syntax-checked; a live CEF run could not be verified in this environment because the chromium link build fails on pre-existing Zig-0.16 libc++ header errors reproduced on untouched examples, and macOS `gpu_surface` views are themselves still gated to the system engine). Live proof on the system engine: `examples/canvas-preview` renders a native canvas toolbar/sidebar and a WKWebView on real `https://` URLs in one window, with Msg-driven navigation, resize re-snapping, and automation-snapshot webview URLs (`url="..."` now printed for webview views); `zig build test-canvas-preview-smoke` verifies it end to end.
- **Menu-bar extra (`UiApp.Options.status_item`)**: a chrome-level status-bar item — macOS `NSStatusItem`, live-verified — installed once on the installing frame: title and/or template icon, tooltip, and menu items whose `command` names dispatch through the ordinary `on_command` mapping with source `.tray` (window-less command sources now dispatch against the canvas window instead of failing). `TrayOptions` grew a `title` so a status item can be a text extra (`NSVariableStatusItemLength`) rather than icon-only; both macOS hosts (AppKit and CEF) render it. Platforms without a status-bar service log a warning and continue. `examples/canvas-preview` installs one; a real System Events click on its "Reload Preview" item drove a model update end to end.
...
- **Effect timers (`fx.startTimer` / `fx.cancelTimer`)**: TEA apps can finally start a timer — `fx.startTimer(.{ .key = refresh_timer, .interval_ms = 3 * 60 * 1000, .mode = .repeating, .on_fire = Fx.timerMsg(.refresh_tick) })` from `init_fx` or `update`, `fx.cancelTimer(key)` to stop (unknown keys are a no-op). Timers ride the platform timer service under framework-reserved ids (their own 16-slot table and key namespace — no effect slots, no worker threads), fire as ordinary Msgs (`zero_native.EffectTimer`), and follow the platform contract: starting an active key replaces it in place. One-shot and repeating modes; rejection is never silent (a full table, a zero interval, or a platform without timers delivers exactly one Msg with outcome `.rejected`). The fake executor records timer requests (`pendingTimerAt`) and tests fire them by hand (`fireTimer(key)`), fully deterministic. Previously `Options.on_timer` existed but `runtime.startTimer` was unreachable from TEA code, so even a boring auto-refresh had no supported wiring.
- **Builder render channel: `opacity` and `transform` on `ElementOptions`**: `ui.el(.stack, .{ .transform = canvas.Affine.translate(dx, 0), .opacity = 0.5 }, ...)` wraps the widget's emitted commands in the display list's existing opacity/transform groups — the authoring seam for animated nav push/pop and fades. Identity/1 defaults emit nothing (existing frames stay byte-identical); layout is untouched (siblings do not reflow) while pointer hit-testing follows the transform, and opacity 0 culls painting but not hit-testing (pair with `disabled` when fading interactive content). Pairs with `UiApp.Options.animations` for tweening; the channel below `Widget` (equality, invalidation, fingerprints, packet serialization, macOS packet host, reference/software renderers) was already threaded and is now proven end to end by tests from the builder through the presented packet JSON.
- **Markup avatars can show runtime images**: `<avatar image="{user.avatar_image}">{initials}</avatar>` binds a `u64` `canvas.ImageId` from a model field or pub fn (arena fns included) — the id is just data, so the closed grammar holds; `0` keeps the initials fallback, and the bound image clips to the avatar circle (`cover` fit) exactly like `ui.avatar`. Literals are rejected with a teaching error (runtime image ids are model data, not markup literals), `image` on any other element is an error (the other image-bearing widgets stay Zig views), and both engines, the CLI validator, and the LSP (completion + diagnostics) enforce the same rules. Previously an app whose list pane was markup could never show the avatars its Zig panes fetched.
...
- **`gap` on stacking containers is a teaching error**: `panel`, `stack`, `card`, and the overlay/surface containers layer their children on top of each other, so `gap` silently did nothing — the classic "why do my picker rows overlap" bug. Markup now rejects `gap` on those elements in both engines, the CLI validator, and the LSP ("gap does nothing here: this container layers its children on top of each other - wrap them in a column (or row) inside it for flow, or drop the gap"), and the Zig builder logs the same lesson as a Debug-build `zero_canvas_ui` warning (never a build failure — shipped apps carry the mistake). New public predicate `canvas.widgetKindStacksChildren` keeps the element list and the layout engine in lockstep.
- **Layout-capacity failures are loud in tests**: an effects-wake drain whose rebuild overflowed the per-view widget budget used to apply the model update, fail the rebuild into the #38 error ring, and leave the previous tree on screen — correct live behavior, but fully silent in a `TestHarness` run (model asserts pass, tree asserts fail mysteriously). The runtime gains a `dispatch_error_policy` (`.degrade` in production, unchanged; the TestHarness sets `.propagate` so recorded dispatch errors also fail the dispatching test call), and `UiApp` logs a teaching diagnostic on `WidgetLayoutListFull` naming the view and the per-view budget (`canvas_limits.max_canvas_widget_nodes_per_view`).
- **Automation widget commands can no longer kill the app**: a `widget-click`/`widget-action` against a stale or unmounted target (virtualized rows below the fold, a click racing a rebuild) used to escape the frame callback as `error.InvalidCommand` and exit the whole app (`CallbackFailed`) — indistinguishable from a crash in the change under test. Automation command errors now route into the #38 dispatch-error ring under stable names (`automation.widget_click`, ...) and publish in the next snapshot; the app keeps running. Driver misuse is deliberately exempt from the test-time propagate policy — it must never fail the app under test.
- **`gpu_present_path` in automation snapshots**: gpu_surface view lines now carry `gpu_present_path=none|packet|pixels` — which presentation path last actually painted (stamped only on successful presents; idle skips keep the last value). Previously the `canvas_frame_*` metrics reflected the last *planned* frame, so a frame silently evicted from the packet path to the software pixels fallback (the I1 image regression) was only provable with an OS screenshot. (`gpu_present_mode` was already taken by the surface's timer/on-demand mode, hence the `_path` name.)
