...
View commands are available through `window.zero.views.*` when `js_window_api` is `true`. They use the same origin and `window` permission checks as window and WebView commands, and `windowId` must match the calling window when provided. `kind: "webview"` routes through the WebView backend. The macOS system-WebView backend supports native `toolbar`, `titlebar_accessory`, `sidebar`, `statusbar`, `button`, `checkbox`, `toggle`, `text_field`, `search_field`, `label`, `spacer`, and `progress_indicator` views; other platform hosts and native kinds return explicit unsupported-backend errors until implemented.
