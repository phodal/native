...
A zero-native app returns a `zero_native.App` value:
...
const App = struct {
    fn app(self: *@This()) zero_native.App {
        return .{
            .context = self,
            .name = "my-app",
            .source = zero_native.WebViewSource.html("<h1>Hello</h1>"),
            .source_fn = source,
            .start_fn = start,
            .event_fn = event,
            .stop_fn = stop,
...
```zig
zero_native.WebViewSource.html("<p>Inline</p>")
zero_native.WebViewSource.url("http://127.0.0.1:5173/")
zero_native.WebViewSource.assets(.{
...
This is useful for mobile hosts, game engines, custom render loops, and headless tests. The repository includes iOS and Android examples that link `libzero-native.a` through Swift/Kotlin host apps.
