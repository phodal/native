...
The keyboard rides the same composition with no extra wiring: on the focused trigger, Enter, Space, or an arrow key presses it (the model-owned open); with the menu mounted, Down/Up move into and walk the options (entering at the `selected` one), Home/End jump to the edges, Enter picks the focused option, and Escape dismisses — focus returns to the trigger either way.
