...
Platform support queries are available through `window.zero.platform.supports(...)` when `js_window_api` is `true`. The command accepts feature names such as `native_views`, `tray`, `clipboard_rich_data`, and `gpu_surfaces`; JavaScript callers can also use camelCase aliases such as `nativeViews`, `clipboardRichData`, and `gpuSurfaces`. Use an explicit `builtin_bridge` policy when you want per-command origin lists.
