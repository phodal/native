...
`zig build model-contract` (part of every scaffolded app's build; `zig build test` refreshes it too) reflects your `Model`/`Msg` into `zig-out/model-contract.zon`. With that artifact fresh, `native check` verifies markup against your app's actual surface — no app compile in the loop:
...
A missing or stale artifact (the artifact carries a hash of your Zig sources) degrades to structural checking with a note — never a false pass; the compiled engine still enforces the same contract at build time, and a conformance suite holds the two checkers to identical accept/reject sets.
