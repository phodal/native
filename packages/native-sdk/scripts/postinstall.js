...
const binaryName = `native-sdk-${platformKey}${ext}`;
...
async function fixUnixSymlink() {
  ...
  const symlinkPath = join(npmBinDir, 'native');
...
async function fixWindowsShims() {
  ...
  const cmdShim = join(npmBinDir, 'native.cmd');
  const ps1Shim = join(npmBinDir, 'native.ps1');

  if (!existsSync(cmdShim)) return;

  const cpuArch = arch() === 'arm64' ? 'arm64' : 'x64';
  const relativeBinaryPath = `node_modules\\@native-sdk\\cli\\bin\\native-sdk-win32-${cpuArch}.exe`;
