...
- **Per-effect line bounds (`max_line_bytes`)**: `.lines` spawns and `.stream` fetches can raise the 4 KiB delivered-line bound up to a 64 KiB ceiling (`max_effect_line_bytes_ceiling`) — agent CLIs emit whole events as single NDJSON lines that the default cap destroyed (a long `claude -p --output-format stream-json` answer arrived as unrecoverable truncation). Bounds above the default heap-allocate the effect's line buffer plus a per-line transfer allocation only for oversized lines; over-ceiling (or zero) requests are rejected through the effect's terminal Msg, never silently clamped; lines beyond the granted bound still arrive truncated and flagged. Verified with a real process emitting a >4 KiB single line delivered byte-exact.
