...
## Resource descriptor URLs

Bridge resource descriptors contain `zero://native/resource/*` URLs. Treat these URLs as short-lived capabilities: do not log them, persist them, or pass them to untrusted content. Native resource handlers still enforce the descriptor's origin, window, TTL, and one-shot settings, but leaking the descriptor gives any same-scope document a chance to consume it. For provider streams, TTL bounds unfetched descriptors; once a permitted fetch starts, the stream can finish.

If your app uses a restrictive Content Security Policy, allow resource fetches from `zero://native` in the relevant fetch directives. Prefer consuming descriptors immediately through `window.zero.resources` and then discarding them.
