...
For product UI, prefer Native SDK's [Built-in Components](/built-in-components). They are native-rendered through the retained canvas/GPU surface, use shadcn-style defaults, and stay portable across desktop and mobile hosts. Native controls remain useful for shell chrome, menus, platform dialogs, and OS-owned integration points.
