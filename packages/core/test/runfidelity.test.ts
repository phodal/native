    ...
    node: `
{
  ...
  const enc = (cmd) => {
    ...
    if (cmd.op === "image_load") {
      return [16, ...f64le(cmd.id), kinds.indexOf(cmd.eventKind), ...long(Array.from(cmd.path)), ...long(Array.from(cmd.url)), ...long(Array.from(cmd.cachePath)), ...f64le(cmd.expectedBytes)];
    }
    return cmd.cmds.flatMap(enc);
  };
  const step = (model, msg) => { const r = mod.update(model, msg); return Array.isArray(r) ? [r[0], Uint8Array.from(enc(r[1]))] : [r, new Uint8Array(0)]; };
