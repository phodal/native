...
test "image_load wire records match rt.zig's documented layout" {
    ...
    // image_load, local path: [0x10][id f64 LE][event_tag][path][url]
    // [cache][expected f64 LE].
    const load = dispatch(.load, &log);
    try std.testing.expectEqual(@as(u8, 0x10), load[0]);
    ...
    try std.testing.expectEqual(@as(u8, 0x10), stream[0]);
    ...
    // image_cancel: [0x11][id f64 LE] — the id is a model expression
    // (the adopted cover, 7).
    const drop = dispatch(.drop, &log);
    try std.testing.expectEqual(@as(u8, 0x11), drop[0]);
    ...
    // image_unregister: [0x12][id f64 LE] — same one-field body, its own
    // opcode (registry surgery, not a load verb).
    const evict = dispatch(.evict, &log);
    try std.testing.expectEqual(@as(u8, 0x12), evict[0]);
