...
`button-group` is a row container that visually attaches related action buttons. For an exclusive active segment use a [toggle-group](/components/toggle) (pressed state is the toggle family's job) or [tabs](/components/tabs).
