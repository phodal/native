...
extern fn zero_native_gtk_register_resource_bytes(host: *GtkHost, id: [*]const u8, id_len: usize, mime: [*]const u8, mime_len: usize, bytes: [*]const u8, bytes_len: usize, origin: [*]const u8, origin_len: usize, window_id: u64, ttl_ns: i64, has_expiry: c_int, one_shot: c_int) c_int;
...
fn registerResourceBytes(context: ?*anyopaque, id: []const u8, mime: []const u8, bytes: []const u8, origin: []const u8, window_id: platform_mod.WindowId, ttl_ns: ?i128, one_shot: bool) anyerror!void {
    ...
    try platform_mod.checkResourceRegistrationResult(zero_native_gtk_register_resource_bytes(self.host, id.ptr, id.len, mime.ptr, mime.len, bytes.ptr, bytes.len, origin.ptr, origin.len, window_id, platform_mod.optionalDurationForC(ttl_ns), if (ttl_ns != null) 1 else 0, if (one_shot) 1 else 0));
}

fn registerResourceStream(context: ?*anyopaque, registration: platform_mod.ResourceStreamRegistration) anyerror!void {
    const self: *LinuxPlatform = @ptrCast(@alignCast(context.?));
    if (self.web_engine == .chromium) return error.UnsupportedService;
    try platform_mod.checkResourceRegistrationResult(zero_native_gtk_register_resource_stream(
        self.host,
        registration.id.ptr,
        registration.id.len,
