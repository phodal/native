...
zero-native provides built-in commands for app command routing (`zero-native.command.*`), windows (`zero-native.window.*`), generic native views (`zero-native.view.*`), layered WebViews (`zero-native.webview.*`), dialogs (`zero-native.dialog.*`), selected OS capabilities (`zero-native.os.*`), clipboard access (`zero-native.clipboard.*`), and credential storage (`zero-native.credentials.*`). These are controlled separately from app-defined commands via the `builtin_bridge` field in `RuntimeOptions`.
