...
The Native SDK provides headless testing tools for bridge and lifecycle coverage without a GUI, plus automation-based integration tests. For running these tiers on GitHub Actions — including the workflow `native init` scaffolds — see [Testing in CI](/testing/ci).
...
```zig
var harness: native_sdk.TestHarness = undefined;
harness.init(.{});
```
