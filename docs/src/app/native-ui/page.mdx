...
| `icon` | built-in vector icon leaf | `name` picks one of the curated Lucide-style icons (literal, compile-checked; 41 names — transport, files/folders, arrows/chevrons, theming, GitHub vocabulary); tint with `foreground`, size with `width`/`height`; never a hit target — decoration inside pressable rows passes clicks through |
