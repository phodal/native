...
Markdown builds on the same model. `zero_native.markdown` maps a GitHub-flavored subset — headings, inline styles, links (including bare `http(s)://` URLs, which autolink with trailing punctuation trimmed), bullet/ordered/task lists, fenced code blocks, blockquotes, rules, and `<details>` collapsibles — onto ordinary widgets:
